...
namespace TemTemArena.Scripts.GUI
{
    class ScreenBuffer
    {
            ...
            foreach (var messages in commands)
            {
                foreach (var message in messages)
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        _screenBuffer[yBuffer * (ScreenWidth + 1) + xBuffer] = "\x1b[48;5;" + color + $"m{message[i]}";
        ...
        void LoadText(int left, int right, int top, int bottom, IEnumerable<string[]> commands, bool useColor, Align alignment)
        {
            ...
            foreach (var messages in commands)
            {
                foreach (var message in messages)
                {
                    ...
                    for (var i = 0; i < length; i++)
                    {
                        ...
                        var col = useColor ? Colors.GetColor(DefaultTextBackgroundColor) : Colors.GetColor(DefaultBackgroundColor);
                        _screenBuffer[yBuffer * (ScreenWidth + 1) + xBuffer] = "\x1b[48;5;" + col + $"m{message[i]}";
